...
## babel-preset-taro[​](babel-config.html#babel-preset-taro)
`babel-preset-taro` 会根据当前项目的技术栈，选择性地使用以下的 `presets` 和 `plugins`。
#### 1. 通用[​](babel-config.html#1-通用)
##### presets[​](babel-config.html#presets)

- `@babel/preset-env`
- `@babel/preset-typescript`（TypeScript 环境）
##### plugins[​](babel-config.html#plugins)

- `@babel/plugin-transform-runtime`
- `@babel/plugin-proposal-decorators`
- `@babel/plugin-proposal-class-properties`
- `babel-plugin-dynamic-import-node`（小程序环境）
#### 2. React[​](babel-config.html#2-react)
##### presets[​](babel-config.html#presets-1)

- `@babel/preset-react`
##### plugins[​](babel-config.html#plugins-1)

- `react-refresh/babel`
#### 3. Vue[​](babel-config.html#3-vue)
##### presetes[​](babel-config.html#presetes)

- `@vue/babel-preset-jsx`
#### 4. Vue3[​](babel-config.html#4-vue3)
##### plugins[​](babel-config.html#plugins-2)

- `@vue/babel-plugin-jsx`
以下将详细介绍 `babel-preset-taro` 的配置项。
### reactJsxRuntime[​](babel-config.html#reactjsxruntime)
备注
只在使用 **React** 时生效。
**默认值**：`'automatic'`
`@babel/preset-react` 的 [runtime](https://babeljs.io/docs/en/babel-preset-react#runtime) 配置项。
### ts[​](babel-config.html#ts)
备注
使用 Typescript
### hot[​](babel-config.html#hot)
备注
只在使用 **React** 时生效。
**默认值**：`true`
...
#### useBuiltIns: 'usage'[​](babel-config.html#usebuiltins-usage)
信息
**优点**：按需引入、不会影响全局变量。
...
### absoluteRuntime[​](babel-config.html#absoluteruntime)
